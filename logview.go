...
//
// For examples typical Java exception looks like
...
// First event to be matched is the one after the event id equal to lastEventId. If the lastEventId is an empty
...
func (lv *LogView) FindMatchingEvent(lastEventId string, predicate func(event *LogEvent) bool) *LogEvent {
    ...
    event := lv.findByEventId(lastEventId)
    if event.previous != nil && event.next != nil {
        event = event.next
    }
    for event != nil {
...
func (lv *LogView) Draw(screen tcell.Screen) {
    ...
    width = width - 6 // APJ fix for full screen border  // APJ for border
    if height == 0 {
        return
    }
    lv.screenCoords[0] = x
